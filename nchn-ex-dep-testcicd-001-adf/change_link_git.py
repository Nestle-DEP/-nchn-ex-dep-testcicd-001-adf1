...
def config(name, env, link, path):
    try:
        ...
        armPath = rootPath + "/linkedTemplates"
        masterArm = rootPath + "/linkedTemplates/ArmTemplate_master.json"
        dirsPath = rootPath + "/dirs/"
...
def readMasterArm(masterArm, masterFile):
    with open(masterArm, 'r', encoding='utf-8') as file_message:
        ...
        dict1 = read_old_message['resources']
        for key in dict1:
            for x in key['properties']['parameters']:
                if x != 'factoryName' and x.find("trigger") < 0:
                    filename = "ArmTemplate_{0}.json".format(key['name'].split('_')[2])
                    a = "%s-%s" % (filename, x)
                    masterFile.append(a)
    ...
    for root, dirs, files in os.walk(armPath):
        for f in files:
            ...
            with open(oldpath, 'r', encoding='utf-8') as file_message:
                read_old_message = json.loads(file_message.read())
                # 区分读取ADF名称
                if f == "ArmTemplateParameters_master.json":
                ...
                oldPrimaryPath = ''
                str_message = ''
                if f == "ArmTemplateParameters_master.json" or f == "ArmTemplate_master.json":
                ...
                if oldSecondaryPath != '':
                    str_message = str_message.replace(oldSecondaryPath, newSecondaryPath)
                if oldHDI != '':
                    str_message = str_message.replace(oldHDI, newHDI)
                str_message = str_message.replace(oldadfname, newadfname) \
